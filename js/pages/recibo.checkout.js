      ...
      <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px;padding:8px 0;${idx < carrito.length - 1 ? 'border-bottom:1px solid rgba(55,123,76,0.3)' : ''}">
        <div style="display:flex;gap:8px;flex:1;min-width:0">
          <div style="display:flex;flex-direction:column;align-items:center;flex-shrink:0">
            <img src="${imgSrc}" alt="${it.name}" style="width:64px;height:auto;object-fit:contain;border-radius:8px;filter:drop-shadow(0 2px 6px rgba(0,0,0,0.10))" onerror="this.onerror=null;this.src='/imagenes/no-image.png'">
            ${it.weight || it.grind || it.presentation ? `<span style="font-family:'Poppins',sans-serif;font-size:0.7rem;color:#50453e;margin-top:4px;text-align:center;line-height:1.2">${it.presentation ? it.presentation.replace('1lb','1 lb (454 g)') : ''}${it.weight ? it.weight + 'g' : ''}${(it.presentation || it.weight) && it.grind ? ' · ' : ''}${it.grind || ''}</span>` : ''}
          </div>
