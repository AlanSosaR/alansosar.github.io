/* ============================================================
   FINANZAS — EGRESOS | CAFÉ CORTERO
   ============================================================ */

console.log("📉 finanzas/egresos.js — INIT");
...
(() => {
  ...
  const CATEGORIAS = ["Todos", "Gasolina", "Aceite motor", "Mano de obra", "Gas tostadora", "Insumos", "Mantenimiento", "Empaque", "Tienda", "Otros"];
  ...
  function fmtMontoHTML(n, signo) {
    ...
    parts[0] = parts[0].replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    const s = signo || "";
    return `${s ? `<span style="color:var(--md-error);font-weight:900;">${s}</span>` : ""}<span style="color:var(--md-error);font-weight:900;">${parts.join(".")}</span> <span style="color:var(--marron);font-weight:600;font-size:0.6em;vertical-align:super;">HNL</span>`;
  }
  ...
  function renderResumen(data) {
    ...
    const el = document.getElementById("egresos-total");
    ...
    (async () => {
      ...
      const signo = diff >= 0 ? "+" : "";
      const elComp = document.getElementById("egresos-compare");
      if (elComp) elComp.textContent = `${signo}${pct}% vs período anterior`;
    })();
  }

  function renderLista(data) {
    const container = document.getElementById("egresos-lista");
    if (!container) return;
    ...
    if (pageItems.length === 0) {
      container.innerHTML = `
        <div class="fin-empty">
          ...
          <div class="fin-empty-title">${q ? "Sin resultados de búsqueda" : "Sin egresos en este período"}</div>
          <div class="fin-empty-desc">${q ? "Intentá con otros términos" : "Registrá tu primer egreso"}</div>
        </div>`;
      return;
    }
      ...
      grupos[key].forEach((item, idx) => {
        html += `
          <div class="fin-item" data-id="${item.id}">
            <div class="fin-item-leading error">
  ...
  function toggleDetail(el, item) {
    ...
    btns[1].addEventListener("click", async () => {
      if (!await showConfirmSnackbar("¿Eliminar este egreso?")) return;
      const { error } = await sb.from("finanzas_movimientos").delete().eq("id", item.id);
      if (!error) {
        el.remove();
        div.remove();
        showSnackbar("Egreso eliminado");
      } else {
