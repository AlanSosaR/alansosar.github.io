...
(() => {
  ...
  function fmtMontoHTML(n, signo) {
    ...
    parts[0] = parts[0].replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    const s = signo || "";
    return `${s ? `<span style="color:var(--md-error);font-weight:900;">${s}</span>` : ""}<span style="color:var(--md-error);font-weight:900;">${parts.join(".")}</span> <span style="color:var(--verde);font-weight:600;font-size:0.6em;vertical-align:super;">HNL</span>`;
  }
    ...
    if (filtered.length === 0) {
      container.innerHTML = `
        <div class="fin-empty">
          ...
          <div class="fin-empty-title">Sin egresos en este período</div>
          <div class="fin-empty-desc">Registrá tu primer egreso</div>
        </div>`;
      return;
    }
    ...
    div.innerHTML = `
      <div class="fin-item-detail-inner">
        <div class="fin-detail-header">
          <span class="material-symbols-outlined" style="color:var(--md-error);font-size:22px;">trending_down</span>
